...
LOR_LEVEL_MAPPING = { level['code']: level for level  in LOR_LEVELS }
...
def convert_to_rdf(graph, source_folder, level):
    ...
    level_acronym = LOR_LEVEL_MAPPING[level_code]['acronym']
    container_code = LOR_LEVELS[level-1]['code']
    container_acronym = LOR_LEVEL_MAPPING[container_code]['acronym']
    ...
    for feature in features:
        ...
        # "gml_id": "s_lor_prog.0702"
        lor_id = properties['gml_id'].split('.').pop()
        feature_name = f"{level_acronym}_{lor_id}"
        title_attribute = LOR_LEVEL_MAPPING[level_code]['title'].lower()
        feature_title = properties[title_attribute]
        feature_res = lor19[feature_name]
        feature_geometry = json.dumps(feature['geometry'])
        feature_type = unit[LOR_LEVEL_MAPPING[level_code]['title']]
        ...
        container_name = f"{container_acronym}_{container_id}"
        container_res = lor19[container_name]
...
base = "https://berlinonline.github.io/lod-berlin-lor-2019/"
base21 = "https://berlinonline.github.io/lod-berlin-lor/"
graph = Graph()
schema = Namespace("https://schema.org/")
geo = Namespace("http://www.opengis.net/ont/geosparql#")
lor19 = Namespace(base)
lor21 = Namespace(base21)
...
for code, title in BEZIRKS_MAPPING.items():
    ...
    bezirks_res = lor19[bezirks_name]
    
    geojson_path = args.source / "bezirke.geojson"
